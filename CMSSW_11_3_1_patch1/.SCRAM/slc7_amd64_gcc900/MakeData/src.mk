...
ROOTCLING:=$(ROOTRFLX_BASE)/bin/rootcling
endif
...
subdirs_src+=$(filter-out Documentation, src_Configuration src_RecoMuon src_Analysis src_RecoTracker)
...
$(COMMON_WORKINGDIR)/cache/project_links:
	@echo '>> Creating project symlinks';\
	[ -d $(@D) ] ||  $(CMD_mkdir) -p $(@D) &&\
	config/SCRAM/createSymLinks.sh src/LCG include/LCG 1 . '' &&\
	if [ ! -f $@ ] ; then touch $@; fi
